...
struct BridgeRenderer {
  ...
  // (FDV_TryAcquirePresentSurface) 每 must be atomic.
...
static bool CreateDeviceAndSurface(BridgeRenderer *s) {
  ...
  hr = s->device->CreateRenderTarget(
      ...
      FALSE, // lockable=FALSE 每 D3DImage does not need CPU lock
...
static void SetupRenderState(IDirect3DDevice9 *dev) {
  ...
  // ignored 每 we skip them entirely.
...
static bool ExecuteCommands(BridgeRenderer *s, const uint8_t *data, int bytes) {
  ...
  while (p < end) {
    ...
    // ------------------------------------------------------------------
    case CMD_FILL_RECT: // [x,y,w,h : 4℅f32][A,R,G,B]
    {
      if (p + 20 > end)
        goto done;
      float x = ReadF32(p);
    ...
    // ------------------------------------------------------------------
    case CMD_STROKE_RECT: // [x,y,w,h : 4℅f32][thickness:f32][A,R,G,B]
    {
      if (p + 24 > end)
        goto done;
      float x = ReadF32(p);
    ...
    // ------------------------------------------------------------------
    case CMD_FILL_ELLIPSE: // [cx,cy:2℅f32][rx,ry:2℅f32][A,R,G,B]
    {
      if (p + 20 > end)
        goto done;
      float cx = ReadF32(p);
...
__declspec(dllexport) bool __cdecl FDV_SubmitCommands(void *renderer,
  ...
  if (hr == D3DERR_DEVICENOTRESET) {
    ...
    return false; // D3DERR_DEVICELOST 每 wait for next call
  }
...
__declspec(dllexport) void __cdecl FDV_OnFrontBufferAvailable(void *renderer,
  ...
  if (!available) {
    // Surface is going away 每 mark as not ready so TryAcquirePresentSurface
